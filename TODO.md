...
- [ ] Fix remaining broken test call sites (app runtime is clean; tests only): tests/test_agent_coordination.py:96,105 add ai_engine= to VideoScenePrompt; tests/test_tapri_guard.py:77,86 replace tone="x" with a valid tone key. (The tests/test_character_and_creative_scenes.py video_verification= additions are no longer needed — ReelScript.video_verification is Optional since 2026-09-24.)
...
- [x] core/models.py: ReelScript.video_verification now Optional[VideoPassVerification] = None (2026-09-24): removing the gate's kwarg would otherwise crash Stage 5 (VideoPassVerification has fail-loud required fields, no defaults). None = gate did not run, never an invented "passed".
