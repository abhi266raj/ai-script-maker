...
- **FR-12.3 (All-Stage Numbering):** Stage 1: 1.1 verify → 1.2 validate (1.2.1 news check, 1.2.2 confidence check) → 1.3 retry → 1.4 re-validate. Stage 2: 2.1 generate characters → 2.2 validate (2.2.1 count check, 2.2.2 diversity check) → 2.3 retry → 2.4 re-validate. Stage 4: 4.1 derive scenes → 4.2 validate (4.2.1 location check — ⚙️ code validator) → 4.3 retry → 4.4 re-validate. Stage 5: 5.1 generate storyboards → 5.2 validate (5.2.1 realism & coherence — ⚙️ code validator, blocking; the old advisory AI quality gate was REMOVED 2026-09-24 as a paid always-pass call) → 5.3 retry → 5.4 re-validate. `ReelScript.video_verification` is `Optional[VideoPassVerification] = None` since the removal — None means the gate did not run (honest), never an invented default "passed" (the model's fields stay fail-loud required). There is no Stage 6 — integration is the automatic Finalize step (FR-4.6).
