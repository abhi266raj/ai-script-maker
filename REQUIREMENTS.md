...
- **FR-1.2:** In **Continuous Mode**, the pipeline functions uninterrupted, streaming telemetry across all 6 stages and presenting the final screenplay upon completion.
...
- **FR-3.9 Stage 3 AI-Judge Validation (News Coverage + Tone):** Regex/token validators are brittle for cross-script (English↔Devanagari) matching and tone semantics. Stage 3 uses AI judges as the semantic authority: (1) `ai_judge_news_coverage` — asks the AI directly whether the dialogue states the news; short-form mentions, paraphrases, and transliterations count; generic filler fails. (2) `ai_judge_tone_compliance` — asks the AI whether the tone is maintained (e.g. 70%+ genuinely funny beats for comedy; zero jokes for lament); returns a specific issue description. When a judge finds non-compliance, the specific issue is fed back to the AI for one corrective regeneration on the exact failed draft. Final gate fails loudly if still broken after correction. News coverage is validated per preview script against ITS OWN hook (multi-preview batches).
...
- **FR-4.6 Stage 6 (Integration & Final Validation Gate):**
  - FIRST integrates every stage output (verified facts, finalized characters, dialogue-derived scenes, dialogue script, storyboards, video prompts, audit trail) into one coherent final package; THEN runs the validation gate.
  - Validation checks every configured value, cross-stage connectivity, and reports issues as a numbered list with fix guidance — never as fake success.
  - Validation failures are reported as "Script validation found issues" — NEVER as "Script generation failed."
  - Displays: Full configuration compliance audit, validation findings, and the final production screenplay package.

#### 2.5 Final Production Deliverables
- **FR-5.1:** Upon completing Step 6, the studio provides the identical full production outputs as Continuous mode:
...
sequenceDiagram
    ...
    alt Continuous Mode
        ...
        CE->>SubAgents: Execute Stages 1 -> 6 continuously
        ...
        loop Stage by Stage (Steps 1 to 6)
...
- **FR-11.6 (Stage 6 Detection vs Source-Stage Attribution):** A runtime crash while Stage N executes is reported as "failed at Stage N". A Stage 6 validation finding about earlier output is reported as "detected at Stage 6; Stage N output needs fixing" — using each issue's `stage` field. `ReelBatchResult` carries `validation_passed` and `validation_issues`; the UI lists them numbered with detection stage, source stage, script number, check, detail, and required fix. Validation failures NEVER render as "Stage None".
...
- **FR-11.9 (Continuous-Mode Stage Progress + Previous Output):** Continuous mode shows **Stage X/6** progress using the pipeline's own `stage_label` (e.g. "Stage 1 of 6: …") in the status header, plus a progress bar. Every completed stage's output accumulates in collapsible "✓ Stage N output" expanders (verified facts + confidence, finalized characters, dialogue counts, scenes) that stay visible while later stages generate.
...
- **FR-12.2 (Stage 3 Numbering):** 3.1 dialogue generation (input → output); 3.2 validation containing exactly four checks — 3.2.1 Structure, 3.2.2 News Coverage, 3.2.3 Tone, 3.2.4 Language; 3.3 retry generation (only on 3.2 failure); 3.4 re-validation (only after 3.3).
- **FR-12.3 (All-Stage Numbering):** Stage 1: 1.1 verify → 1.2 validate (1.2.1 news check, 1.2.2 confidence check) → 1.3 retry → 1.4 re-validate. Stage 2: 2.1 generate characters → 2.2 validate (2.2.1 count check, 2.2.2 diversity check) → 2.3 retry → 2.4 re-validate. Stage 4: 4.1 derive scenes → 4.2 validate (4.2.1 connectivity check) → 4.3 retry → 4.4 re-validate. Stage 5: 5.1 generate storyboards → 5.2 validate (5.2.1 quality gate) → 5.3 retry → 5.4 re-validate. Stage 6: 6.1 integrate → 6.2 validate (6.2.1 integration checks) → 6.3 retry → 6.4 re-validate.
...
- **FR-12.5 (Validate Where Created):** Each stage validates the content IT creates. Stage 6 keeps ONLY integration-level checks (required outputs exist, cross-stage IDs/counts/connectivity match, integration payload non-empty). Stage 6 MUST NOT re-validate Stage 3 tone/news/style quality already validated upstream — that wastes model calls and tokens.
...
- **FR-12.7 (Check Order by Failure Likelihood):** Sub-checks are ordered most-prone-to-fail FIRST, so fail-fast catches the likeliest problem immediately. Stage 3: 3.2.1 Structure → 3.2.2 News Coverage → 3.2.3 Tone → 3.2.4 Language (language least prone, checked last). The same ordering principle applies to every stage's validation step.
...
- **FR-13.3 (Substep Status Indicators):** Current substep shows 🔄 running; completed substeps show ✅/❌; pending substeps show ⏳ waiting. Each substep is collapsible, expanding to its input → output. Applies to all 6 stages.
...
- **FR-14.2:** Every substep remains collapsible with its input/output visible, so the user can inspect exactly what succeeded and what broke. Applies to all 6 stages.
