...
- **Continuous vs Step-Wise Script Generation Mode:**
  - **Continuous Mode (⚡ Automated):** Executes the full 5-stage pipeline without interruption, streaming real-time status updates across all stages and presenting the final complete screenplay upon completion.
  - **Step-Wise Mode (🪜 Interactive Control):** Breaks execution into distinct checkpoints (Stage 1: Fact Validation & Intelligence Dossier ➔ Stage 2: Viral Angles & Hindi Hooks ➔ Stage 3: Spoken Hindi Dialogue & Timing Calibration ➔ Stage 4: Scene Storyboards & 9:16 Video Prompts ➔ Stage 5: Final Review & Chief Editor Sign-Off).
    - **Intermediate Stage Inspection:** Displays verified wire facts, formulated hooks, dialogue lines, storyboard beats, and video prompts at each stage.
    - **Dynamic Instruction Refinement (Tickbox):** Checkbox toggle allows creators to inject extra custom instructions to either re-run/refine the current step or guide the subsequent step.
    - **Per-Step Model / Engine Override:** Creators can choose or change the AI engine independently for each individual step (e.g. Codex for fact-checking, Grok for viral hooks, Antigravity for screenplay dialogue, Local FM for storyboard directions).
    - **Final Production Screenplay:** Step 5 completes with the identical comprehensive multi-tab screenplay, teleprompter, clean text, and download assets.
    - **Same-Stage Correction Feedback & Dialogue Interconnectedness:** When re-running a step (e.g. Stage 3 Dialogues), the pipeline automatically extracts the previous draft and feeds it to the agent alongside user correction feedback in a dedicated `# 🔄 REVISION & CORRECTION MODE` block. Dialogue prompts strictly mandate reactive ping-pong, interruptions, keyword callbacks, and direct emotional counterpoints so characters talk directly to each other rather than making isolated declarations.
    - **Testing Isolation Policy:** Default automated tests and CI suites run strictly against the local on-device model (`fm_only` / local inference bridge) with zero network dependency on live external endpoints. Tests for other models (Grok, Codex, AGY cloud) are strictly conditional and run only when needed to triage or fix a model-specific bug.
